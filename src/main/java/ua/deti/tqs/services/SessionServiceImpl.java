...
public class SessionServiceImpl implements SessionService {
  ...
  public List<Session> getAllSessionsByStationId(int stationId) {
    ...
    List<Session> sessions =
        sessionRepository.findAllByChargingSpot_Station_Id(stationId).orElse(List.of());

    if (sessions.isEmpty()) {
      log.debug("No sessions found with station id {}", stationId);
    }

    log.debug("Found {} sessions with station id {}", sessions.size(), stationId);
    return sessions;
  }
  ...
  public List<Session> getAllSessionsByUserId(int userId) {
    ...
    log.debug("Found {} sessions with user id {}", sessions.size(), userId);
    return sessions;
  }

  @Override
  public Session getSessionById(int session) {
    log.debug("Fetching session with id {}", session);
    Session existingSession = sessionRepository.findById(session).orElse(null);
    if (existingSession == null) {
      log.debug("No session found with id {}", session);
      return null;
    }
    log.debug("Session with id {} found", session);
  ...
  public Session createSession(int userId, Session session) {
    ...
    if (vehicle == null) {
      ...
      log.debug(
          "Invalid session vehicle, user id {} does not match vehicle user id {}",
          userId,
    ...
    ChargingSpot chargingSpot =
        chargingSpotRepository.findById(session.getChargingSpot().getId()).orElse(null);

    if (chargingSpot == null) {
      log.debug(
          "Invalid session charging spot, charging spot with id {} not found",
          session.getChargingSpot().getId());
      return null;
    }
    ...
    if (sessions != null) {
      for (Session s : sessions) {
        ...
        if (s.getStartTime().isBefore(startTime.plusSeconds(duration))
            && s.getStartTime().plusSeconds(s.getDuration()).isAfter(startTime)) {
          log.debug(
              "Invalid session charging spot, charging spot with id {} is already in use",
              session.getChargingSpot().getId());
          return null;
        }
      }
    ...
    newSession.setPaymentStatus(PaymentState.PENDING);

    BigDecimal power = chargingSpot.getPowerKw();
    BigDecimal hours =
        BigDecimal.valueOf(session.getDuration())
  ...
  // TODO TEST THIS
  @Override
  public Session updatePaymentStatus(int userId, int sessionId, PaymentState paymentStatus) {
    log.debug("Updating payment status for session with id {}", sessionId);
    Session existingSession = sessionRepository.findById(sessionId).orElse(null);
    if (existingSession == null) {
      log.debug("No session found with id {}", sessionId);
      return null;
    }
    if (existingSession.getVehicle().getUser().getId() != userId) {
      log.debug("User {} isn't the owner of the session with id {}", userId, sessionId);
      return null;
    }
    if (paymentStatus == null) {
      log.debug("Invalid session payment status, payment status is null");
      return null;
    }
    existingSession.setPaymentStatus(paymentStatus);
    log.debug("Payment status for session with id {} updated to {}", sessionId, paymentStatus);
  ...
  public boolean deleteSession(int userId, int sessionId) {
    ...
    if (session.getVehicle().getUser().getId() != userId) {
      log.debug(
          "The user with id {} is not the owner of the session with id {}", userId, sessionId);
      return false;
    }
